...
export const WorkActivityCharts = ({ activeCycle, cycles, issues, states }: { activeCycle?: PLANE_CYCLE; issues: PLANE_ISSUE[]; states: PLANE_STATE[]; cycles: PLANE_CYCLE[] }) => {
	...
	const percentageIssueDone = (noDoneIsues / noIssues) * 100 || 0;
	const percentageIssuePending = (noPendingIsues / noIssues) * 100 || 0;

	return (
		<div className="mt-20 flex gap-20">
			<div className="w-full">
				<div className="relative h-1 w-full rounded-md bg-accent">
					<div className="absolute bottom-0 left-0 top-0 z-10 rounded-md bg-foreground transition-all duration-500" style={{ width: percentageIssueDone + '%' }}>
					...
					<div className="absolute bottom-0 left-0 top-0 rounded-md bg-orange-300 transition-all duration-500" style={{ width: percentageIssueDone + percentageIssuePending + '%' }}>
						{percentageIssueDone + percentageIssuePending > 0 && <div className={cn('absolute -right-px bottom-0 border-r pr-2 text-xs text-muted-foreground', 50 > 0 ? 'h-16' : 'h-12')}>{percentageIssuePending}%</div>}
					</div>
				</div>
				...
				<ul className="mt-8 space-y-1">
					{groupedIssues.map(state => (
						<Fragment key={state.state + state.issues}>
							<li key={state.state + state.issues} className="flex items-center justify-between rounded-md py-3 text-sm font-light transition-all duration-500 hover:bg-accent/50 hover:pl-2 hover:pr-4">
