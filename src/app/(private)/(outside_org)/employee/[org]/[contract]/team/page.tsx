...
export default async function ProfilePage({ params }: { params: { [key: string]: string } }) {
	...
	return data.team && (!data.terminated_by || (data.end_date && !isPast(data.end_date))) && <Teams isManager={!!manager?.length} currentUser={'profile'} name={data.team.name} contractId={data.id} org={params.org} team={data.team.id} />;
}
