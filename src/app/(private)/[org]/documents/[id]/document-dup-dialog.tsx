...
export const DocumentDupDialog = ({ children, onClose, document, currentUserId }: { document: Tables<'documents'>; children: ReactNode; onClose: (state: boolean) => void; currentUserId: string }) => {
	...
	const makeCopy = async () => {
		...
		const payload: TablesInsert<'documents'> = copyConfigs ? { ...document, name: documentName, owner: currentUserId } : { name: documentName, json: document.json, org: document.org, editors: [currentUserId] };
