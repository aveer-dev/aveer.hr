...
export default async function RolePage({ params }: { params: { [key: string]: string }; searchParams: { [key: string]: string } }) {
	...
	let { data, error } = await supabase.from('open_roles').select('*, entity:legal_entities!open_roles_entity_fkey(id, name, incorporation_country)').match({ id: params.role, org: params.org_id }).single();
	...
	const deleteRole = async (): Promise<string> => {
		...
		const { data, error } = await supabase.from('profiles_roles').select().match({ organisation: params.org_id, profile: user.id });
		if (error || !data.length) return `You do not have adequate org permission to terminate contracts`;

		const { error: roleError } = await supabase.from('open_roles').delete().match({ org: params.org_id, id: params.role });
		if (roleError) return roleError.message;

		return redirect(`/${params.org_id}/open-roles/`);
	};
		...
		<section className="mx-auto -mt-6 grid max-w-4xl gap-10 p-6 pt-0">
			<div className="flex justify-between">
				<div className="flex gap-8">
					<Link href={`/${params.org_id}/open-roles`} className={cn(buttonVariants({ variant: 'outline', size: 'icon' }), 'rounded-full')}>
						<ChevronLeft size={12} />
					</Link>
				...
				<div className="flex gap-3">
					<>
						<ToggleRoleStatus status={data?.status} org={params.org_id} role={params.role} />
						...
						<Popover>
							...
							<PopoverContent align="end" className="w-48 p-2">
								<Link href={`./${params.role}/edit`} className={cn(buttonVariants({ variant: 'ghost', size: 'sm' }), 'w-full justify-start gap-2')}>
									<FilePenLine size={12} />
									Edit Role
								</Link>
								<Link href={`./new?duplicate=${params.role}`} className={cn(buttonVariants({ variant: 'ghost', size: 'sm' }), 'w-full justify-start gap-2')}>
									<Copy size={12} />
									Duplicate
								</Link>
