...
export const AddPerson = ({ data, duplicate }: { data?: TablesUpdate<'contracts'>; duplicate?: TablesUpdate<'contracts'> }) => {
	...
	const getEntities = useCallback(async () => {
		const { data, error } = await supabase.from('legal_entities').select().eq('org', params.org_id);
	...
	const createEORAgreement = async ({ org }: { org: number }) => {
		const supabase = createClient();

		const { data, error } = await supabase.from('org_documents').insert({ name: 'EOR Contract', org, type: 'eor' }).select('id').single();
		if (error) return toast.error(error.message);
		if (data) setAgreementId(data.id);
	};

	const doesUserHaveAdequatePermissions = async () => {
		const {
			data: { user }
		...
		const role = await supabase
			.from('profiles_roles')
			.select()
			.match({ organisation: Number(params.org_id), profile: user?.id });
		if (role.error) return toast.error(role.error.message);
		if (role.data && !role.data.length) return toast.error(`You do not have adequate org permission to sign contracts`);
		return true;
	};

	const onSubmit = async (values: z.infer<typeof formSchema>) => {
		const userHasPermission = await doesUserHaveAdequatePermissions();
		if (userHasPermission !== true) return;
