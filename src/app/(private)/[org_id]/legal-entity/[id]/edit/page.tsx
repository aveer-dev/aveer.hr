...
export default async function EditEntityPage({ params }: { params: { [key: string]: string }; searchParams: { [key: string]: string } }) {
	...
	const { data, error } = await supabase.from('legal_entities').select().match({ org: params.org_id, id: params.id }).single();
	...
	return (
		<div className="mx-auto max-w-4xl">
			...
			<Suspense
				...
				{data && <LegalEntityForm orgId={Number(params.org_id)} data={data} />}
			</Suspense>
		</div>
	);
}
