...
export const useTextmenuContentTypes = (editor: Editor) => {
	return useEditorState({
		...
		selector: (ctx): ContentPickerOptions => [
			...
			{
				icon: 'Heading1',
				onClick: () => ctx.editor.chain().focus().lift('taskItem').liftListItem('listItem').setHeading({ level: 1 }).run(),
				id: 'heading1',
			...
			{
				icon: 'Heading2',
				onClick: () => ctx.editor.chain().focus().lift('taskItem').liftListItem('listItem').setHeading({ level: 2 }).run(),
				id: 'heading2',
			...
			{
				icon: 'Heading3',
				onClick: () => ctx.editor.chain().focus().lift('taskItem').liftListItem('listItem').setHeading({ level: 3 }).run(),
				id: 'heading3',
