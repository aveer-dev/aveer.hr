			...
			{withPortal ? (
				...
				<Popover.Portal className="z-9999">
					<Popover.Content asChild sideOffset={8}>
						<Surface className="z-[9999] flex max-h-80 min-w-[15rem] flex-col gap-0.5 overflow-auto p-2">{children}</Surface>
					</Popover.Content>
				</Popover.Portal>
			) : (
				<Popover.Content asChild sideOffset={8}>
...
export const Item = ({ label, close = true, icon, iconComponent, disabled, onClick, isActive }: { label: string | React.ReactNode; icon?: keyof typeof icons; iconComponent?: React.ReactNode; close?: boolean; disabled?: boolean; onClick: () => void; isActive?: boolean }) => {
	const className = cn(
...
export const CategoryTitle = ({ children }: CategoryTitle) => {
	return <div className="mb-1.5 mt-4 select-none px-1 text-[0.625rem] font-medium uppercase text-neutral-400 first:mt-1.5 dark:text-neutral-600">{children}</div>;
};
