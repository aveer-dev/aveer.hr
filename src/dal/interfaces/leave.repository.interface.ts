...
export interface ILeaveRepository {
	...
	getByContract(org: string, contract: number, status?: Tables<'time_off'>['status']): Promise<Tables<'time_off'>[]>;
	...
	getAllByOrgWithRelations(org: string, status?: Tables<'time_off'>['status']): Promise<LeaveWithRelations[]>;
	getByProfileWithRelations(org: string, profile: string, status?: Tables<'time_off'>['status']): Promise<LeaveWithRelations[]>;
	getByContractWithRelations(org: string, contract: number, status?: Tables<'time_off'>['status']): Promise<LeaveWithRelations[]>;
}
