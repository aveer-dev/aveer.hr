...
Deno.serve(async (req) => {
    ...
    const { data } = await supabase
        .from("contracts")
        .select("profile:profiles!contracts_profile_fkey(fcm_token)")
        .match({ "org": payload.record.org, status: "signed" });

    const fcmTokens = data!.map((d) =>
    ...
    const notification = {
        ...
        icon:
            "https://api.aveer.hr/storage/v1/object/public/platform%20assets/logo/aveer-round.png",
    };
